...
- (void) layoutSubviews
{
    ...
    if (indicator == nil) {
        _value = CGPointMake(CGRectGetMidX(self.bounds) + 85, CGRectGetMidY(self.bounds));
...
-(void) hueCal;
{
    ...
    CGFloat alpha = asin( (pcenter.y - _value.y)/85 );
...
-(void) setHue:(CGFloat)newHue
{
    ...
    {
        _hue = newHue;
        CGFloat  y =  CGRectGetMidY(self.bounds) - CircleRadius * sin(_hue * PI2);
        CGFloat x =  CGRectGetMidX(self.bounds) + CircleRadius * cos(_hue * PI2);
...
- (void) trackIndicatorWithTouch: (UITouch*) touch
{
    ...
    self.value =  CGPointMake(sx + (85.0 / dis) * (point.x-sx), sy + (85.0 / dis) * (point.y-sy));
